...
static const int M5Stack_LCD_MOSI = 23;
static const int M5Stack_LCD_MISO = -1;
static const int M5Stack_LCD_SCLK = 18;
static const int M5Stack_LCD_CS   = 14;
static const int M5Stack_LCD_DC   = 27;
static const int M5Stack_LCD_RESET= 33;
static const int M5Stack_LCD_BL   = 32;
...
void spi_master_init(TFT_t * dev)
{
	...
	ESP_LOGI(TAG, "M5Stack_LCD_CS=%d",M5Stack_LCD_CS);
	gpio_pad_select_gpio( M5Stack_LCD_CS );
	gpio_set_direction( M5Stack_LCD_CS, GPIO_MODE_OUTPUT );
	gpio_set_level( M5Stack_LCD_CS, 0 );

	ESP_LOGI(TAG, "M5Stack_LCD_DC=%d",M5Stack_LCD_DC);
	gpio_pad_select_gpio( M5Stack_LCD_DC );
	gpio_set_direction( M5Stack_LCD_DC, GPIO_MODE_OUTPUT );
	gpio_set_level( M5Stack_LCD_DC, 0 );

	ESP_LOGI(TAG, "M5Stack_LCD_RESET=%d",M5Stack_LCD_RESET);
	if ( M5Stack_LCD_RESET >= 0 ) {
		gpio_pad_select_gpio( M5Stack_LCD_RESET );
		gpio_set_direction( M5Stack_LCD_RESET, GPIO_MODE_OUTPUT );
		gpio_set_level( M5Stack_LCD_RESET, 0 );
		vTaskDelay( pdMS_TO_TICKS( 100 ) );
		gpio_set_level( M5Stack_LCD_RESET, 1 );
	}

	ESP_LOGI(TAG, "M5Stack_LCD_BL=%d",M5Stack_LCD_BL);
	if ( M5Stack_LCD_BL >= 0 ) {
		gpio_pad_select_gpio( M5Stack_LCD_BL );
		gpio_set_direction( M5Stack_LCD_BL, GPIO_MODE_OUTPUT );
		gpio_set_level( M5Stack_LCD_BL, 0 );
	}

	spi_bus_config_t buscfg = {
		.sclk_io_num = M5Stack_LCD_SCLK,
		.mosi_io_num = M5Stack_LCD_MOSI,
		.miso_io_num = M5Stack_LCD_MISO,
		.quadwp_io_num = -1,
		.quadhd_io_num = -1
	};
	...
	assert(ret==ESP_OK);
	dev->_dc = M5Stack_LCD_DC;
	dev->_bl = M5Stack_LCD_BL;
...
{
	dev->_width = M5STACK_LCD_WIDTH;
	dev->_height = M5STACK_LCD_HEIGHT;
	dev->_offsetx = M5STACK_LCD_OFFSET_X;
	dev->_offsety = M5STACK_LCD_OFFSET_Y;
